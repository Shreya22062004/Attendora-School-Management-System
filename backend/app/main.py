...
def additive_migrate():
 ...
 with engine.begin() as c:
  statements=[
   ...
   'ALTER TABLE students ADD COLUMN IF NOT EXISTS photo_storage_key VARCHAR',
   'ALTER TABLE students ADD COLUMN IF NOT EXISTS photo_storage_url VARCHAR',
...
def seed():
 db=SessionLocal()
 try:
  school_id_row=db.query(School.id).first()
  school_id=school_id_row[0] if school_id_row else None
  if not school_id:
   school=School(school_name='GOVERNMENT NODAL UPS KANTABANJI',address='BLOCK_TUREKELA DIST_BALANGIR ODISHA PIN_767039',udise_code='21241600701');db.add(school);db.flush();school_id=school.id
