...
const deleteTeacher=async t=>{if(!confirm(`Delete teacher ${t.username}? This removes the teacher login and class assignments. Existing attendance records will remain.`))return;try{const r=await api.delete(`/auth/teachers/${t.id}`);setMsg(r.data.message);if(editingTeacher===t.id){setEditingTeacher(null);setTeacher({username:'',password:'',classes:[]})}load()}catch(e){setMsg(e.response?.data?.detail||'Could not delete teacher')}};
