...
const load=()=>{api.get('/academic/years').then(r=>setYears(r.data)).catch(e=>setMsg(e.response?.data?.detail||'Could not load academic years'));api.get('/settings/config').then(r=>setClasses(r.data.classes||[]));api.get('/students').then(r=>setStudents(r.data));api.get('/auth/teachers').then(r=>setTeachers(r.data)).catch(()=>{});api.get('/academic/audit').then(r=>setLogs(r.data)).catch(()=>{})};useEffect(load,[]);
