...
export default function IDCards() {
  ...
  const [students, setStudents] = useState([]), [classes, setClasses] = useState([]), [settings, setSettings] = useState({});
  const [classFilter, setClassFilter] = useState(""), [search, setSearch] = useState(""), [year, setYear] = useState("");
  ...
  const load = async () => { try { const [studentResponse, configResponse, settingsResponse] = await Promise.all([api.get("/idcards/students"), api.get("/settings/config"), api.get("/idcards/settings")]); setStudents((studentResponse.data.students || []).map(item => ({ ...item, kind: "student" }))); setClasses(configResponse.data.classes || []); setSettings(settingsResponse.data); setYear(settingsResponse.data.established_year || ""); } catch (error) { setMessage(error.response?.data?.detail || "Could not load ID cards"); } };
  useEffect(() => { load(); }, []);
  const people = useMemo(() => students.filter(person => { const classMatch = !classFilter || person.class_name === classFilter; const term = search.trim().toLowerCase(); const searchMatch = !term || [person.name, person.contact_number, person.mobile_number, person.class_name].some(value => String(value || "").toLowerCase().includes(term)); return classMatch && searchMatch; }), [students, classFilter, search]);
  const preview = people.find(person => `${person.kind}-${person.id}` === previewKey) || people[0];
  const uploadedPhotoCount = people.filter(person => person.photo_uploaded).length;
  const pendingPhotoCount = people.length - uploadedPhotoCount;
  ...
  const printCards = async selected => {
